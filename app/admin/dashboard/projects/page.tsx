  ...
  try {
    ...
    if (pendingEditImage) {
      const base64 = await new Promise<string>((resolve) => {
        const reader = new FileReader()
        reader.readAsDataURL(pendingEditImage)
    ...
    const res = await fetch(`/api/projects/${editId}`, {
      ...
      body: JSON.stringify({
        ...rest,
        image: imageUrl,
        technologies: rest.technologies.split(",").map((t: string) => t.trim()),
      }),
    })
    if (res.ok) {
    ...
    <div>
      ...
      <div className="space-y-4">
        ...
        {projects.map((project) => (
          <div
            key={project.id}
            className="bg-slate-800 border border-white/10 rounded-xl p-6 flex justify-between items-center"
          >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              accept="image/*"
              onChange={(e) =>
                e.target.files?.[0] && setPendingEditImage(e.target.files[0])
              }
              className="w-full px-4 py-3 bg-slate-700 text-white rounded-lg border border-white/10 outline-none"
            />
          </div>
          <Field
            ...
            onChange={(v) => setEditForm({ ...editForm, technologies: v })}
          />
          <div className="grid grid-cols-2 gap-4">
            <Field
              label="Live URL"
              value={editForm.liveUrl}
              onChange={(v) => setEditForm({ ...editForm, liveUrl: v })}
            />
            <Field
              label="GitHub URL"
          ...
          <Field
            ...
            onChange={(v) => setEditForm({ ...editForm, order: Number(v) })}
            type="number"
          />
