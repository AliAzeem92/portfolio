...
export default function ProjectsAdminPage() {
  ...
  // Silent refresh — no loader flash after initial load
  ...
  const handleAddImageUpload = async (file: File) => {
    setUploading(true);
    try { setForm((p) => ({ ...p, image: "" })); const url = await uploadImage(file); setForm((p) => ({ ...p, image: url })); }
  ...
  const handleEditImageUpload = async (file: File) => {
    ...
    try { const url = await uploadImage(file); setEditForm((p) => ({ ...p, image: url })); }
    catch { showToast("Upload failed.", "error"); }
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm(emptyForm);
        fetchData(true); // silent — no loader
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/projects/${editId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...rest, technologies: rest.technologies.split(",").map((t: string) => t.trim()) }),
      });
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Optimistic update — remove from state immediately, no reload
