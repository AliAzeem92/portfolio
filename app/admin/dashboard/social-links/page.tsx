  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-semibold text-white mb-4">Existing Links</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {links.map((link) => (
            <div key={link.id} className="bg-slate-800 border border-white/10 rounded-xl p-6 flex flex-col gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-white font-medium truncate">{link.label}</p>
                <p className="text-gray-400 text-sm break-all line-clamp-2">{link.href}</p>
              </div>
              <div className="flex gap-2 mt-auto">
                <button onClick={() => handleEdit(link)} className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm transition-colors">Edit</button>
                <button onClick={() => handleDelete(link.id)} className="flex-1 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-sm transition-colors">Delete</button>
              </div>
            </div>
          ))}
        </div>
